...
const getFilteredComponents = (excludeComponents = [], components) => {
  return sortBy(components, (component) => component.tagName).filter(
    (component) => !excludeComponents.includes(component.tagName) && !component.internal
  );
};
const getPathToCorePackageLoader = (config, outputTarget) => {
  ...
  const distAbsEsmLoaderPath = distOutputTarget?.esmLoaderPath && path__namespace.isAbsolute(distOutputTarget.esmLoaderPath) ? distOutputTarget.esmLoaderPath : null;
  const distRelEsmLoaderPath = config.rootDir && distAbsEsmLoaderPath ? path__namespace.relative(config.rootDir, distAbsEsmLoaderPath) : null;
  const loaderDir = outputTarget.loaderDir || distRelEsmLoaderPath || DEFAULT_LOADER_DIR;
  return normalizePath(path__namespace.join(basePkg, loaderDir));
};
...
const generateProxies = (config, components, pkgData, outputTarget, rootDir) => {
  const distTypesDir = path__namespace.dirname(pkgData.types);
  const dtsFilePath = path__namespace.join(rootDir, distTypesDir, GENERATED_DTS);
...
`;
  const generateTypeImports = () => {
    if (outputTarget.componentCorePackage !== void 0) {
...
`;
  ...
  if (outputTarget.includeImportCustomElements && outputTarget.componentCorePackage !== void 0) {
...
const copyResources = async (config, outputTarget) => {
  ...
  const srcDirectory = path__namespace.join(__dirname, "solid-component-lib");
  const destDirectory = path__namespace.join(path__namespace.dirname(outputTarget.proxiesFile), "solid-component-lib");
...
const normalizeOutputTarget = (config, outputTarget, validate = true) => {
  ...
  if (outputTarget.directivesProxyFile && !path__namespace.isAbsolute(outputTarget.directivesProxyFile)) {
    results.proxiesFile = normalizePath(path__namespace.join(config.rootDir, outputTarget.proxiesFile));
  }
  return results;
};
...
exports.solidOutputTarget = solidOutputTarget;
